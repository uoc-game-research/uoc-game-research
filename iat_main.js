...
//Global Variables
//
var iatConfiguration;
//Configuration data
var blockNb = new Array();
var serverURL;
var postTrialPause;
 ...
 */
var rightKey = 'KeyI'; // 73;	// I Key (event.code=KeyI)
var leftKey = 'KeyE'; // 69;	// E Key (event.code=KeyE)
...
function loadConfig() {
	...
	$.when(
		...
		$.get('iat_config.json', function(data) {

			/* PBo: IMPORTANT!
			JSON.parse(data); is not necessary if the file is a .json file
			correctly written. */
			// configIAT = JSON.parse(data);
			iatConfiguration = data;

			//Initialize global variables for config information
			serverURL = configIAT.serverURL;
			blockNb = configIAT.blockNumber;
			language = configIAT.language;
			introScreenText = configIAT.introScreen;
			instructionScreenText = configIAT.instructionScreen;
			endScreenText = configIAT.endScreen;
			postTrialPause = configIAT.postTrialPause;
		})
	).then(function() {
...
function introScreen() {
	...
	console.log("Prearing context to display instructions");
	var ctx = prepareContext(document.getElementById("mainCanvas").getContext("2d"));
	...
	function iatStart(width, height) {
		...
		window.addEventListener('keydown', function(e) {
			...
			if (e.code == leftKey && (correctCategory == categories.Position1)) {
				...
				experimentData.push(String(blockNb[0]) + "," + categories.Position1 + "," + categories.Position2 + "," +
...
//for each.
function blockController(blockNb) {

	if (blockNb.length > 0) {

		var blockName = blockNb[0];
